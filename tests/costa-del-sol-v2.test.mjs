...
const expectedMunicipalities=["Algarrobo","Almuñécar","Benalmádena","Casares","Estepona","Fuengirola","Málaga","Manilva","Marbella","Mijas","Nerja","Rincón de la Victoria","Torremolinos","Torrox","Vélez-Málaga"];

test("el catálogo v2.1 contiene 63 playas válidas en 15 municipios",()=>{assert.equal(beaches.length,63);assert.deepEqual([...new Set(beaches.map(beach=>beach.municipality))].sort(),expectedMunicipalities.sort());assert.equal(new Set(beaches.map(beach=>beach.id)).size,63);assert.equal(new Set(beaches.map(beach=>beach.slug)).size,63);for(const beach of beaches){assert.ok(beach.latitude>=36.3&&beach.latitude<=36.8,beach.id);assert.ok(beach.longitude>=-5.3&&beach.longitude<=-3.6,beach.id);assert.equal(beach.autonomousCommunity,"Andalucía",beach.id)}});

test("AEMET y Google Maps cubren las 63 playas",()=>{assert.deepEqual(Object.keys(AEMET_MUNICIPALITY_CODES).sort(),expectedMunicipalities.sort());assert.equal(AEMET_MUNICIPALITY_CODES["Almuñécar"],"18017");for(const beach of beaches){assert.ok(AEMET_MUNICIPALITY_CODES[beach.municipality],beach.id);const url=new URL(getGoogleMapsDirectionsUrl(beach.latitude,beach.longitude));assert.equal(url.hostname,"www.google.com");assert.equal(url.searchParams.get("destination"),`${beach.latitude},${beach.longitude}`)}});
...
test("el snapshot sanitario cubre 61 playas y solo deja dos unknown",()=>{assert.equal(Object.keys(sanitaryRecords).length,61);const unknown=beaches.filter(beach=>!(beach.id in sanitaryRecords)).map(beach=>beach.id).sort();assert.deepEqual(unknown,["malaga-misericordia","mijas-el-faro"]);assert.equal(sanitaryRecords["torrox-las-dunas-la-carraca"].sanitaryAssociation,"associated");assert.match(sanitaryRecords["torrox-las-dunas-la-carraca"].message,/Cenicero–Las Lindes/);for(const id of ["almunecar-la-herradura","almunecar-cotobro","almunecar-san-cristobal","almunecar-puerta-del-mar","almunecar-velilla"]){assert.equal(sanitaryRecords[id].sanitaryAssociation,"individual",id);assert.match(sanitaryRecords[id].sourceUrl,/maritimas_granada_20260814\.pdf$/,id)}});
...
test("Open-Meteo se divide en cuatro lotes paralelos y un fallo no cancela los demás",async()=>{const batches=splitIntoBatches(beaches,20);assert.deepEqual(batches.map(batch=>batch.length),[20,20,20,3]);let started=0;let release;const gate=new Promise(resolve=>{release=resolve});const task=runIndependentBatches(batches,async(_batch,index)=>{started+=1;await gate;if(index===1)throw new Error("fallo simulado");return index});await new Promise(resolve=>setImmediate(resolve));assert.equal(started,4);release();assert.deepEqual(await task,[0,undefined,2,3])});

test("Almuñécar usa metadata de Granada y coordenadas oficiales sin duplicados",()=>{const expected={"almunecar-la-herradura":[36.733183,-3.744979],"almunecar-cotobro":[36.733056,-3.714722],"almunecar-san-cristobal":[36.731,-3.699059],"almunecar-puerta-del-mar":[36.730375,-3.68666],"almunecar-velilla":[36.73797,-3.668216]};for(const[id,coordinates]of Object.entries(expected)){const beach=beaches.find(item=>item.id===id);assert.ok(beach,id);assert.equal(beach.municipality,"Almuñécar");assert.equal(beach.province,"Granada");assert.equal(beach.autonomousCommunity,"Andalucía");assert.deepEqual([beach.latitude,beach.longitude],coordinates)}});
