...
async function loadRealData():Promise<Beach[]> {
 ...
 const [marineByBeach,sanitary]=await Promise.all([seaPromise,sanitaryPromise]);
 ...
 return catalog.map(base=>{
  ...
  const beach:Beach={id:base.id,slug:base.slug,name:base.name,municipality:base.municipality,latitude:base.latitude,longitude:base.longitude,
   ...
   sources:{
    ...
    jellyfish:{state:"coming-soon",origin:"unknown",label:"Medusas",message:"Fuente en proceso de integración."},occupancy:{state:"coming-soon",origin:"unknown",label:"Ocupación",message:"Fuente en proceso de integración."},
   }};
