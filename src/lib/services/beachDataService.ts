...
const SNAPSHOT_KEY="all-beaches-v22-63";
...
async function persistRuntimeSnapshot(snapshot:BeachDataSnapshot){const rejection=weatherCoverageRejection(snapshot,WEATHER_MUNICIPALITIES);if(rejection){console.warn("[beachSnapshot] candidate rejected; keeping last valid snapshot",rejection);return false}memorySnapshot=snapshot;try{const cache=getCache({namespace:"playa-hoy"});const version=Date.parse(snapshot.refreshedAt).toString(36);const chunks=Array.from({length:Math.ceil(snapshot.beaches.length/SNAPSHOT_CHUNK_SIZE)},(_,index)=>snapshot.beaches.slice(index*SNAPSHOT_CHUNK_SIZE,(index+1)*SNAPSHOT_CHUNK_SIZE));const chunkKeys=chunks.map((_,index)=>`${SNAPSHOT_KEY}:${version}:${index}`);await Promise.all(chunks.map((chunk,index)=>cache.set(chunkKeys[index],chunk,{ttl:SNAPSHOT_TTL_SECONDS,tags:["beach-data-snapshot"],name:`Playa Hoy beach data snapshot ${index+1}/${chunks.length}`})));const manifest:BeachDataSnapshotManifest={version:1,referenceTime:snapshot.referenceTime,refreshedAt:snapshot.refreshedAt,chunkKeys};await cache.set(SNAPSHOT_MANIFEST_KEY,manifest,{ttl:SNAPSHOT_TTL_SECONDS,tags:["beach-data-snapshot"],name:"Playa Hoy beach data snapshot manifest"});return true}catch(error){console.warn("[beachSnapshot] Runtime Cache write failed",error instanceof Error?error.message:"error desconocido");return false}}
