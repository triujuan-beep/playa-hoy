...
async function loadRealData():Promise<Beach[]> {
 const weatherArea=(municipality:string)=>municipality==="La Cala de Mijas"?"Mijas":municipality;
 const municipalities=[...new Set(catalog.map(beach=>weatherArea(beach.municipality)))];
 ...
 return catalog.map(base=>{
  const weather=weatherByMunicipality.get(weatherArea(base.municipality));const sea=marineByBeach[base.id];const health=sanitary[base.id];const jellyfish=jellyfishByBeach[base.id];
  ...
  const beach:Beach={id:base.id,slug:base.slug,name:base.name,municipality:base.municipality,latitude:base.latitude,longitude:base.longitude,
   ...
   sanitaryStatus:health.status,sanitaryMessage:health.message,sanitaryEffectiveFrom:health.effectiveFrom,sanitaryEffectiveUntil:health.effectiveUntil,jellyfishObservation:jellyfish,dataMode:"real",hourlyConditions,
...
export const getBeachData=cache(async(idOrSlug:string)=>(await getAllBeachesData()).find(beach=>beach.id===idOrSlug||beach.slug===idOrSlug)??null);
