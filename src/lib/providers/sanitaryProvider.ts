...
function normalize(record?:SanitaryRecord):SanitaryResult{if(!record||!isCurrent(record))return{status:"unknown",message:"No hay un control oficial estructurado y vigente disponible.",source:"Junta de Andalucía · Aguas de baño",sourceUrl:SOURCE_URL};return{...record,source:record.source??"Fuente oficial verificada",sourceUrl:record.sourceUrl}}
